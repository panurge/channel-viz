(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'bNMXcunAs93aNEms1HTmLhatDZrAoKXWb8WvYlsheO28Qntf', // Unique master Xively API key to be used as a default
		defaultFeeds	= [1254033461], // Comma separated array of Xively Feed ID numbers
		applicationName	= 'Central Heating Controller', // Replaces Xively logo in the header
							...
							xively.datastream.history(feedId, datastream.id, {duration: duration, interval: interval, limit: 1000}, function(datastreamData) {
								...
								if(datastreamData.datapoints) {
									...
									var graph = new Rickshaw.Graph( {
										...
										height: 200,
										renderer: 'line',
										min: 10//parseFloat(datastream.min_value) - .25*(parseFloat(datastream.max_value) - parseFloat(datastream.min_value)),
										max: 25//parseFloat(datastream.max_value) + .25*(parseFloat(datastream.max_value) - parseFloat(datastream.min_value)),
										padding: {
											top: 0.02,
											right: 0.02,
											bottom: 0.02,
